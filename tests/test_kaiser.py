...
def test_simple_row(parser):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with zipfile.ZipFile(filename, "w") as artifact:
            artifact.writestr(
                "941105628-san-diego-medical-center-standard-charges-scal-en/KaiserSanDiegoChargeDescriptionMaster.csv",
                rows.encode("utf-8"),
            )
...
def test_artifact_urls(parser):
    assert (
        KaiserChargeMasterParser.artifact_urls == KaiserChargeMasterParser.ARTIFACT_URLS
    )
