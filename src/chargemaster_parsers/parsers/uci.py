            ...
            for category, entries in data_dict.items():
                ...
                if category == "Gross Charges":
                    for entry in entries:
                        ...
                        cash_price = entry.get(
                            "UCI HB OUTPATIENT Discount Cash Price", None
                        )
                        if (
                            uci_hb_full_price != "N/A"
                        ):  # add UCI HB payer entry only if there's a price listed
                        ...
                        if (
                            cash_price != "N/A"
                        ):  # add cash payer entry only if there's a price listed
                            yield ChargeMasterEntry(
                                procedure_identifier=procedure_identifier,
                                procedure_description=procedure_description,
                                hcpcs_code=hcpcs_code,
